...
make_headers(#{host := Host}, ReqOpts) ->
    ...
    F = fun(Name, Acc) ->
            case proplists:is_defined(Name, Acc) of
                true -> Acc;
                false -> set_header(Name, Acc, ReqOpts, Host)
            end
        end,
    lists:foldl(F, Hdrs, [<<"authorization">>, <<"connection">>, <<"host">>]).

set_header(<<"authorization">>, Hdrs, #{auth := #{type := basic} = A}, _Host) ->
    AStr = iolist_to_binary([maps:get(username, A), $:, maps:get(password, A)]),
    [{<<"authorization">>, <<"Basic ", (base64:encode(AStr))/binary>>}|Hdrs];
set_header(<<"authorization">>, Hdrs, _ReqOpts, _Host) ->
    Hdrs;
set_header(<<"connection">>, Hdrs, #{use_pool := true}, _Host) ->
    [{<<"connection">>, <<"keep-alive">>}|Hdrs];
set_header(<<"connection">>, Hdrs, _ReqOpts, _Host) ->
    [{<<"connection">>, <<"close">>}|Hdrs];
set_header(<<"host">>, Hdrs, _ReqOpts, Host) ->
